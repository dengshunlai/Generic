//
//  WaterfallLayoutTestVC3.swift
//  GenericDemo
//
//  Created by 邓顺来1992 on 2024/6/26.
//

import UIKit
import Generic

class WaterfallLayoutTestVC3: ProjBaseViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateWaterfallLayout {
    
    var itemCount: Int = 12
    
    lazy var waterfallCellTemp = WaterfallCell()
    
    lazy var changeWidthBtn = {
        let btn = UIButton.init(type: .custom)
        btn.setTitleColor(UIColor.generic.hexColor("333333"), for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 14)
        btn.setTitle("改变宽度测试", for: .normal)
        btn.addTarget(self, action: #selector(clickChangeWidthBtn), for: .touchUpInside)
        return btn
    }()
    ...
    override func viewDidLoad() {
        ...
        changeWidthBtn.snp.makeConstraints { make in
    ...
    @objc func clickChangeWidthBtn() {
        if changeWidthBtn.isSelected == false {
            collectionView.snp.remakeConstraints { make in
                make.leading.bottom.equalToSuperview()
                make.trailing.equalTo(-150)
                make.top.equalTo(topBar.snp.bottom)
            }
            changeWidthBtn.isSelected = true
        } else {
            collectionView.snp.remakeConstraints { make in
                make.leading.bottom.equalToSuperview()
            ...
            changeWidthBtn.isSelected = false
        }
    }
